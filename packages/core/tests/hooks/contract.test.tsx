...
import { compiledErc20, devnetAccounts } from "../../test/devnet";
import { renderHook, waitFor } from "../../test/react";
import { useContract, useContractFactory } from "./contract";

const address =
  "0x017239d35be9e3a622b01677fff06c05ea7d926b94f864e59188d1a7eca00b1f";

describe("useContract", () => {
  it("returns the contract", () => {
    const { result } = renderHook(() =>
      useContract({ abi: compiledErc20.abi, address }),
    );
  ...
  describe("abi", () => {
    ...
    it("returns undefined if it become undefined", async () => {
      const { result, rerender } = renderHook(
        ({ abi }: { abi?: Abi } = { abi: compiledErc20.abi }) =>
          useContract({ address, abi }),
      );
      expect(result.current.contract).toBeDefined();
  ...
  describe("address", () => {
    it("returns undefined if undefined", () => {
      const { result } = renderHook(() =>
        useContract({ abi: compiledErc20.abi }),
      );
      expect(result.current.contract).toBeUndefined();
    });
