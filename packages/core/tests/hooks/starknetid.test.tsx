...
import { useAddressFromStarkName, useStarkName } from "./starknetid";
...
describe("useStarkName", () => {
  ...
  beforeAll(async () => {
    // rome-ignore lint: figure this one out later
    ...
    const namingResponse = await account.declareAndDeploy({
      ...
      classHash:
        "0x3f2f8c80ab2d404bcfb4182e8528708e4efa2c646dd711bdd7b721ecc6111f7",
    });
    ...
    const idResponse = await account?.declareAndDeploy({
      ...
      classHash:
        "0x1eb5a8308760d82321cb3ee8967581bb1d38348c7d2f082a07580040c52217c",
    });
    ...
    const signed = stark.formatSignature(
      ec.starkCurve.sign(whitelistingPrivateKey, hashed),
    );
  ...
  it.skip("get starkname should fail because contract not deployed", async () => {
    const { result: starkNameResult } = renderHook(() =>
      useStarkName({ address: account.address }),
    );
  ...
  it.skip("get address from starkname should return 0x0 because name does not exist", async () => {
    const { result: addressFromStarkName } = renderHook(() =>
      useAddressFromStarkName({
        name: "starknet-react.stark",
        contract: namingAddress,
      }),
    );
    await waitFor(
      () => {
